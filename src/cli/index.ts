...
async function getTestScenario(): Promise<TestScenario> {
	...
	const browserInput = await askQuestion(
		"ブラウザ（chromium/firefox/webkit、デフォルト: chromium）: ",
	);
	if (
		browserInput &&
		["chromium", "firefox", "webkit"].includes(browserInput)
	) {
		options.browser = browserInput as "chromium" | "firefox" | "webkit";
	}

	const headlessInput = await askQuestion(
	...
	const screenshotInput = await askQuestion(
	...
	const timeoutInput = await askQuestion(
